...
export default function WeaponsPage() {
  ...
  useEffect(() => {
    weaponsApi
      .getAll()
      .then((res: any) => {
        const sorted = (res.data as Weapon[]).sort(
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1 max-w-lg">
            Browse and access detailed intel on all registered Endfield weaponry.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-4">
          {filtered.map((weapon, i) => (
            <Link key={weapon.id} href={`/dashboard/weapons/${weapon.id}`}>
              <motion.div
              ...
              >
                {/* Image Container */}
                <div className="aspect-square bg-[#F7F7F5] relative p-4 flex items-center justify-center">
                  <div
                  ...
                  {/* Weapon Type Label */}
                  <div className="absolute top-2 right-2 bg-black/50 backdrop-blur-sm px-2 py-0.5 rounded text-[9px] uppercase tracking-wider text-white font-mono">
                    {weapon.type}
                  </div>
                </div>
